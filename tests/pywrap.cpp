...
std::vector<double> kza1d_wrap(const std::vector<double> &x,
...
std::vector<std::vector<double>> kza2d_wrap(
                             const std::vector<std::vector<double>> &x,
                             ...
                             unsigned min_window, double tol)
{
    KZA2D<double, size_t, unsigned> kza2d_algo(window, min_window,
                                               tol, iterations);

    return kza2d_algo(x, y);
}
...
PYBIND11_MODULE(libKZ_py, m) {
    ...
    m.def("kza1d", &kza1d_wrap, "A kza1d wrapping function");
    m.def("kza2d", &kza2d_wrap, "A kza2d wrapping function");
}
